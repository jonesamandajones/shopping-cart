...
async def get_address_dict(addresss_collection, user_id):
    try:
...
async def create_address_dict(address_collection, address):
    try:
...
async def get_address_dict_by_id(address_collection, address_dict_id):
    address = await address_collection.find_one({'_id': address_dict_id})
    return address
