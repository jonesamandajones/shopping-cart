...
async def address_crud():
    ...
    if option == '1':
        ...
        address_record = await get_address_record_by_id(address_collection, user_id)
        ...
        if not address_record:
            ...
            add_record['address'].append(address)
            await create_address_record(
                address_collection,
                add_record
            )  
        else:
        ...
        address_record = await get_address_by_id(
            address_collection,
            user_id
        )
        print(address_record)
        for i, add in enumerate(address_record['address']):
            if add['_id'] == add_id:
                index = i
        
        mod_option = (
            input(f'''O endereço escolhido foi: {address_record} \n
        ...
        # pagination
        address_record = await get_address_record(
            address_collection,
            user_id
        )
        
        print(address_record['address'])
