...
async def create_order_item(order_collection, order):
    try:
        order_item = await order_collection.insert_one(order_item)
        ##como diabos inserir o produto nessa lista?!
        order
...
async def add_product(order_collection, user_id, product_data):
    try:
        product = await order_collection.update_one(
            {'_id': user_id},
            {'$push': {f'products': product_data}}
        )
